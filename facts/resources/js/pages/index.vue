...
const listener = (_event: FactReceivedEvent) => {
    router.reload({
        only: ['facts'],
    });
};
...
<template>
    ...
    <main class="mx-auto max-w-4xl px-6 py-10">
        <h1 class="mb-6 text-3xl font-bold">Facts</h1>
        ...
        <ul v-else class="space-y-4">
            ...
            >
                ...
                <h2 class="mb-2 text-lg font-semibold">{{ fact.claim.text }}</h2>
                <p class="mb-3 text-sm text-gray-700">{{ fact.analysis.summary }}</p>

                <p class="mb-2 text-xs font-medium uppercase tracking-wide text-gray-500">Overall verdict</p>
                <p class="mb-4 text-sm font-semibold">{{ fact.overall_verdict }}</p>
